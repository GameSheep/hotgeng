...
async function openDetail(id) {
  ...
  const news = Array.isArray(m.related_news) ? m.related_news : [];
  const kindLabels = { news: '事件报道', analysis: '传播观察', source: '收录依据', collection: '专题盘点' };
  const newsMarkup = news.length ? `<section class="news-dossier"><div class="news-head"><div><span>RELATED SIGNALS</span><h3>相关热点新闻</h3></div><strong>${news.length} 条可核验来源</strong></div><div class="news-timeline">${news.map((item) => `<a class="news-item" href="${esc(item.url)}" target="_blank" rel="noreferrer"><time>${esc(item.published_at || '日期待考')}</time><div><div class="news-meta"><b>${esc(kindLabels[item.kind] || '相关资料')}</b><span>${esc(item.source || '来源待补')}</span></div><h4>${esc(item.title)}</h4><p>${esc(item.summary || '打开查看完整报道与上下文。')}</p></div><i aria-hidden="true">↗</i></a>`).join('')}</div></section>` : `<section class="news-empty"><span>RELATED SIGNALS</span><h3>相关新闻仍在核验</h3><p>编辑部暂未找到可稳定引用的独立报道，因此不会用普通搜索结果冒充热点新闻。</p></section>`;
  const qualityLabel = m.quality === 'detailed' ? '详尽档案' : '待继续校订';
  modalRoot.innerHTML = `<div class="modal-backdrop" id="detailBackdrop"><article class="detail-modal" role="dialog" aria-modal="true" aria-labelledby="detailTitle"><button class="close-modal" id="closeModal" aria-label="关闭档案">×</button><div class="detail-hero"><div class="detail-heading"><div class="detail-kicker">MEME FILE / ${String(m.id).padStart(4, '0')}</div><div class="quality-badge quality-${esc(m.quality)}">${qualityLabel}</div><h2 id="detailTitle">${esc(m.name)}</h2><p class="detail-summary">${esc(m.summary || '这条档案正在补充释义。')}</p><div class="detail-tags">${tagMarkup(m.tags)}</div></div><div class="detail-visual">${visual}</div></div><div class="detail-body"><div class="detail-sections">${[['origin','起源'],['original_meaning','原本含义'],['new_meaning','新的意义'],['usage_scenes','使用场景'],['first_appearance','初次登场地点']].map(([key, label], index) => `<section class="detail-section"><span>${String(index + 1).padStart(2, '0')}</span><h3>${label}</h3><p>${esc(m[key] || '暂无记录')}</p></section>`).join('')}</div>${newsMarkup}<div class="detail-foot"><div class="contributor">ARCHIVED BY / ${esc(m.contributor)}<br>LAST UPDATED / ${esc(String(m.updated_at || '').slice(0, 10))}</div>${me ? '<button class="primary" id="editMeme">提交修订 ↗</button>' : ''}</div></div></article></div>`;
